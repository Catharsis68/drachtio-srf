...
function copyAllHeaders(headers, obj) {
    if (headers)
        Object.keys(headers).forEach((h) => {
            if (!noncopyableHdrs.includes(h) && !obj[h])
                obj[h] = headers[h];
        });
}
function possiblyRemoveHeaders(hdrList, obj) {
    hdrList.forEach((h) => {
        ...
        if (arr) {
            let hdr = arr[1];
            if (!hdr.startsWith('X-') && hdr !== 'Diversion')
                hdr = hdr.toLowerCase();
            delete obj[hdr];
        }
    });
 ...
 */
class Srf extends events_1.EventEmitter {
    /**
    ...
    constructor(app) {
        ...
        if (typeof app === 'function')
            this._app = app; //deprecated
        else if (typeof app === 'string')
            this._tags.push(app);
        else if (Array.isArray(app) && app.every((t) => typeof t === 'string'))
            this._tags = app;
        (0, assert_1.default)(this._tags.length <= 20, 'Srf#constructor: only 20 tags may be supplied');
        (0, assert_1.default)(this._tags.every((t) => t.length <= 32), 'Srf#constructor: tag values must be 32 characters or less');
        (0, assert_1.default)(this._tags.every((t) => /^[a-zA-Z0-9-_+@:]+$/.test(t)), 'Srf#constructor: tag values may only contain characters a-zA-Z0-9-_+@:');
        if (!this._app) {
            this._app = (0, connect_1.default)();
            ...
            if (typeof app === 'object' && !Array.isArray(app)) {
                assert_1.default.equal(typeof app.host, 'string', 'invalid drachtio connection opts');
                const opts = app;
    ...
    connect(opts, callback) {
        ...
        if (this._tags.length)
            args = Object.assign({}, opts, { tags: this._tags });
        return this.app.connect(args, callback);
    }
    listen(opts, callback) {
        if (this._tags.length)
            Object.assign(opts, { tags: this._tags });
    ...
    createUAS(req, res, opts = {}, callback) {
        ...
        (0, assert_1.default)(typeof generateSdp === 'function');
        const __fail = (err, cb) => {
        ...
        const __send = (content, cb) => {
            ...
            req.on('cancel', () => {
                ...
                cb(new sip_error_1.default(487, 'Request Terminated'));
            });
            return res.send(req.method === 'INVITE' ? 200 : 202, undefined, {
                ...
                const dialog = new dialog_1.default(this, 'uas', { req: req, res: res, sent: response, auth: undefined });
                if (req._dialogState) {
                    dialog.stateEmitter = {
    ...
    */
    createUAC(uri, opts, progressCallbacks, callback) {
        ...
        const __x = (cb) => {
            ...
            assert_1.default.ok(method === 'INVITE' || method === 'SUBSCRIBE', 'method must be either INVITE or SUBSCRIBE');
            assert_1.default.ok(!!_opts.uri, 'uri must be specified');
            ...
            if (from) {
                if (!_opts.headers.from && !_opts.headers.From)
                    _opts.headers.from = from;
                if (!_opts.headers.contact && !_opts.headers.Contact)
                    _opts.headers.contact = from;
            }
            const is3pcc = !_opts.localSdp && 'INVITE' === method;
            const launchRequest = (reqUri, reqMethod, reqOpts, launchCb) => {
                    ...
                    req.on('response', (res, ack) => {
                        if (res.status < 200) {
                            ...
                            if (!contact || 0 === contact.length) {
                                const error = new sip_error_1.default(res.status, res.reason);
                                error.res = res;
                                    ...
                                    return launchCb(null, {
                                        ...
                                        ack: (localSdp) => {
                                            return new Promise((resolve, reject) => {
                                                ...
                                                const dialog = new dialog_1.default(this, 'uac', { req: req, res: res, auth: reqOpts.auth, sent: undefined });
                            ...
                            if ((200 === res.status && method === 'INVITE') ||
                                ...
                                if (!res.has('Contact')) {
                                    ...
                                    const error = new sip_error_1.default(500, 'No Contact header in response');
                                    error.res = res;
                                    return launchCb(error);
                                }
                                const dialog = new dialog_1.default(this, 'uac', { req: req, res: res, auth: _opts.auth, sent: undefined });
                                if (req._dialogState) {
                                    dialog.stateEmitter = {
    ...
    */
    createB2BUA(req, res, uri, opts, progressCallbacks, callback) {
        ...
        assert_1.default.ok(typeof _opts.uri === 'string'); // minimally, we must have a request-uri
        ...
        _opts.localSdp = _opts.localSdpB && typeof _opts.localSdpB !== 'function' ? _opts.localSdpB : req.body;
        ...
        const __x = async (cb) => {
            ...
            let uac;
            try {
                const uacOptions = { ..._opts };
                uac = (await this.createUAC(uacOptions, {
                    cbRequest: handleUACSent,
                    cbProvisional: handleUACProvisionalResponse,
                }));
            }
            catch (err) {
    ...
    */
    proxyRequest(req, destination, opts = {}, callback) {
        (0, assert_1.default)(typeof destination === 'undefined' || typeof destination === 'string' || Array.isArray(destination), '\'destination\' is must be a string or an array of strings');
    ...
    * @returns {Srf|Promise} returns a Promise if no callback is supplied, otherwise the Srf object
    ...
    request(socket, uri, opts, callback) {
        ...
        assert_1.default.ok(typeof opts.method === 'string', 'Srf#request: opts.method is required');
        const __x = (cb) => {
            return socket ?
    ...
    addDialog(dialog) {
        ...
        debug('Srf#addDialog: adding dialog with id %s type %s, dialog count is now %d ', dialog.id, dialog.dialogType, this._dialogs.size);
    }
    removeDialog(dialog) {
...
exports.default = Srf;
(0, delegates_1.default)(Srf.prototype, '_app')
    .method('endSession')
    .method('disconnect')
    .method('set')
    .method('get')
...
sip_methods_1.default.forEach((method) => {
    (0, delegates_1.default)(Srf.prototype, '_app').method(method.toLowerCase());
});
