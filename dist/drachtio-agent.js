...
const debug = (0, debug_1.default)('drachtio:agent');
const debugSocket = (0, debug_1.default)('drachtio:socket');
const wire_protocol_1 = __importDefault(require("./wire-protocol"));
const message_1 = __importDefault(require("./sip-parser/message"));
const request_1 = __importDefault(require("./request"));
const response_1 = __importDefault(require("./response"));
const digest_client_1 = __importDefault(require("./digest-client"));
...
const defer = typeof setImmediate === 'function' ?
    setImmediate : function (fn, ...args) { process.nextTick(fn.bind.apply(fn, [null, ...args])); };
function typeSocket(socket) {
    return socket instanceof net_1.default.Socket || socket instanceof tls_1.default.TLSSocket;
}
function sockPort(socket) {
    (0, assert_1.default)(typeSocket(socket));
        ...
        try {
            ...
            if (actual) {
                ...
                if (desired) {
                    ...
                    if (parseInt(actual[1]) > parseInt(desired[1]))
                        return true;
                    if (parseInt(actual[1]) < parseInt(desired[1]))
                        return false;
                    if (parseInt(actual[2]) > parseInt(desired[2]))
                        return true;
                    if (parseInt(actual[2]) < parseInt(desired[2]))
                        return false;
                    if (parseInt(actual[3]) >= parseInt(desired[3]))
                        return true;
                }
                else
                    assert_1.default.ok(false, `failed parsing desired ${minSupportedVersion}`);
            }
            else
                assert_1.default.ok(false, `failed parsing actual ${serverVersion}, please fix`);
        }
        catch (err) {
...
class DrachtioAgent extends events_1.EventEmitter {
    ...
    connect(opts, callback) {
        ...
        this.tags = opts.tags || [];
        this.wp = new wire_protocol_1.default(opts);
        ...
        if (callback) {
            super.on.call(this, 'connect', callback);
        }
    }
    ...
    _makeRequest(params) {
        ...
        const m = new message_1.default(params.options);
        ...
        assert_1.default.ok(typeSocket(params.socket), 'provided socket is not a net.Socket or tls.TLSSocket');
        assert_1.default.ok(params.socket.destroyed !== true, 'provided socket has been destroyed');
        ...
        obj.pendingRequests.set(msgId, (token, msg) => {
            if (token[0] === 'OK') {
                ...
                const req = new request_1.default(new message_1.default(msg), meta);
                req.agent = this;
    ...
    request(...args) {
        const params = this._normalizeParams(args[0], args[1], args[2], args[3]);
    ...
    sendResponse(res, opts, callback, fnAck) {
        ...
        if ((callback && typeof callback === 'function') || fnAck) {
            obj.pendingRequests.set(msgId, (token, msg, meta) => {
                ...
                const responseMsg = new message_1.default(msg);
                res.meta = meta;
                if (callback) {
    ...
    sendAck(method, dialogId, req, res, opts, callback) {
        (0, assert_1.default)(this.mapServer.has(res.socket));
        const obj = this.mapServer.get(res.socket);
        ...
        if (callback) {
            obj.pendingRequests.set(msgId, (token, msg, meta) => {
                ...
                callback(null, new message_1.default(msg));
            });
        }
    }
    ...
    _initServer(socket) {
        (0, assert_1.default)(!this.mapServer.has(socket));
    ...
    _onMsg(socket, msg) {
        ...
        switch (token[1]) {
            case 'sip':
                ...
                try {
                    sipMsg = new message_1.default(rawMsg);
                }
                catch (err) {
                ...
                if (token.length > 9) {
                    ...
                    if ('network' === source && sipMsg.type === 'request') {
                        ...
                        const req = new request_1.default(sipMsg, meta);
                        ...
                        if (obj.pendingSipRequests.has(transactionId)) {
                            ...
                            res = new response_1.default(this);
                            res.msg = sipMsg;
                            res.meta = meta;
                            res.req = sr.req;
                            ...
                            let ack = node_noop_1.noop;
                            if (res.status >= 200 && res.req.method === 'INVITE') {
                                ack = response_1.default.prototype.sendAck.bind(res, token[9]);
                            }
                            else if (res.status > 100 && res.status < 200) {
                                const prackNeeded = res.get('RSeq');
                                if (prackNeeded) {
                                    ack = response_1.default.prototype.sendPrack.bind(res, token[9]);
                                }
                            }
                            ...
                            if (obj.pendingSipAuthRequests.has(cid)) {
                                ...
                                const client = new digest_client_1.default(res);
                                client.authenticate((err, req) => {
                                    ...
                                    if (params && params.options && params.options.stackTxnId) {
                                        debug(`uac-auth: sending out delayed ${params.options.method} originally for ${res.req.stackTxnId}`);
            ...
            case 'cdr:stop':
                ...
                rawMsg = msg.slice(pos + 2);
                const cdrSipMsg = new message_1.default(rawMsg);
                const args = [msgSource, msgTime];
                if (cdrEvent !== 'attempt') {
                    args.push(token[4]);
                }
                args.push(cdrSipMsg.toString());
...
exports.default = DrachtioAgent;
//# sourceMappingURL=drachtio-agent.js.map