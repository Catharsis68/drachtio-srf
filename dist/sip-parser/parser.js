...
function getHeaderName(hdr) {
    ...
    if (process.env.DRACHTIO_PRESERVE_HEADER_NAMES) {
        ...
        if (preservedHeaders.includes(hdr))
            return hdr;
    }
    if (0 === hdr.indexOf('X-') ||
        (0 === hdr.indexOf('P-') && 0 !== hdr.indexOf('P-Asserted')) ||
        -1 !== customHeaderNames.indexOf(hdr))
        return hdr;
...
exports.stringifyAuthHeaderExport = stringifyAuthHeader;
const stringifiers = {
    via: function (h) {
...
function stringifySipMessage(m) {
    let s;
    if (m.status) {
...
function parseSipMessage(s, lazy) {
