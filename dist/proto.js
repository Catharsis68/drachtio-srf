...
const app = {};
app._init = function () {
    const client = new drachtio_agent_1.default(this);
    ...
    ['connect', 'close', 'error', 'reconnecting', 'listening'].forEach((event) => {
        client.on(event, (...args) => {
            events_1.EventEmitter.prototype.emit.apply(this, [event, ...args]);
        });
    });
    ...
    (0, delegates_1.default)(this, 'client')
        .method('request')
        .method('disconnect')
        .method('close')
        .method('get')
    ...
    function next(err) {
        ...
        if (!layer || res.finalResponseSent) {
            ...
            if (err) {
                ...
                const msg = sip_status_1.default[res.status];
