    ...
    modify(sdp, opts, callback) {
        ...
        if (sdpVal) {
            ...
            this._modify(sdpVal, optsVal, (err, res, ack) => {
                if (err)
                    return cb(err);
                cb(null, res.body, ack);
            });
        }
    }
    _modify(sdp, opts, callback) {
                            ...
                            if (res.status >= 200) {
                                if (200 === res.status) {
                                    ...
                                    if (this.local.sdp || opts.noAck !== true) {
                                        ...
                                        return cb(null, res, (sdp) => {
                                            ack({ body: sdp });
                                            onReInviteComplete(this);
                                        });
                                    }
                                }
                                cb(new sip_error_1.default(res.status, res.reason));
                            }
                        });
                    });
                }
                ...
                this.agent.request({
                    ...
                    req.on('response', (res, ack) => {
                        ...
                        if ('INVITE' === method && res.status >= 200) {
                            if (res.status > 200 || opts.body || opts.noAck !== true)
                                ack();
                            else {
