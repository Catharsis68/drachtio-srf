export function getParser(hdr: any) {
  return parsers[hdr] || parseGenericHeader ;
}
export function getStringifier(hdr: any) {
  return stringifiers[hdr];
}
export function getHeaderName(hdr: any) {
...
function parseResponse(rs: any, m: any) {
...
function parseRequest(rq: any, m: any) {
...
function applyRegex(regex: any, data: any) {
...
function parseParams(data: any, hdr: any) {
...
function parseMultiHeader(parser: any, d: any, h: any) {
  h = h || [];
...
function parseAuthenticationInfoHeader(d: any) {
  const a: any = {};
...
const parsers: any = {
  ...
  'cseq': parseCSeq,
  'content-length': function(v: any) { return +v.s; },
...
function parse(data: any, lazy: any) {
  data = data.split(/\r\n(?![ \t])/);
...
export function parseUri(s: any) {
  if (typeof s === 'object')
    return s;
  ...
  if (r) {
    return {
      ...
      port: +r[5],
      params: (r[6].match(/([^;=]+)(=([^;=]+))?/g) || [])
        .map(function(s: any) { return s.split('='); })
        .reduce(function(params: any, x: any) { params[x[0]] = x[1] || null; return params; }, {}),
      headers: ((r[7] || '').split('&').filter((header) => header !== '').reduce((acc: {[key: string]: any}, header: any) => {
...
function stringifyVersion(v: any) {
  return v || '2.0';
}
...
export function stringifyUri(uri: any) {
...
export const stringifyAuthHeaderExport = stringifyAuthHeader;

const stringifiers: any = {
  via: function(h: any) {
    return h.map(function(via: any) {
  ...
  contact: function(h: any) {
    return 'Contact: ' + ((h !== '*' && h.length) ? h.map(stringifyAOR).join(', ') : '*') + '\r\n';
  },
  route: function(h: any) {
    return h.length ? 'Route: ' + h.map(stringifyAOR).join(', ') + '\r\n' : '';
  },
  'record-route': function(h: any) {
    return h.length ? 'Record-Route: ' + h.map(stringifyAOR).join(', ') + '\r\n' : '';
  },
  'path': function(h: any) {
    return h.length ? 'Path: ' + h.map(stringifyAOR).join(', ') + '\r\n' : '';
  },
  cseq: function(cseq: any) {
    return 'CSeq: ' + cseq.seq + ' ' + cseq.method + '\r\n';
  },
  'www-authenticate': function(h: any) {
    return h.map(function(x: any) { return 'WWW-Authenticate: ' + stringifyAuthHeader(x) + '\r\n'; }).join('');
  },
  'proxy-authenticate': function(h: any) {
    return h.map(function(x: any) { return 'Proxy-Authenticate: ' + stringifyAuthHeader(x) + '\r\n'; }).join('');
  },
  'authorization': function(h: any) {
    return h.map(function(x: any) { return 'Authorization: ' + stringifyAuthHeader(x) + '\r\n'; }).join('');
  },
  'proxy-authorization': function(h: any) {
    return h.map(function(x: any) { return 'Proxy-Authorization: ' + stringifyAuthHeader(x) + '\r\n'; }).join('');
  },
  'authentication-info': function(h: any) {
...
export function parseSipMessage(s: any, lazy: any) {
