...
import tls from 'tls';
import { ConnectApp, SrfRequest, SrfResponse } from './types';

const CR = '\r' ;
const CRLF = '\r\n' ;

const defer = typeof setImmediate === 'function' ?
  setImmediate : function(fn: (...args: any[]) => void, ...args: any[]) { process.nextTick(fn.bind.apply(fn, [null, ...args])); } ;

function typeSocket(socket: any): socket is net.Socket | tls.TLSSocket {
  return socket instanceof net.Socket || socket instanceof tls.TLSSocket;
}
function sockPort(socket: net.Socket | tls.TLSSocket): string {
  assert(typeSocket(socket));
  return '' + socket.remotePort + ':' + socket.localPort;
}

function serverVersionAtLeast(serverVersion: string, minSupportedVersion: string): boolean {
...
class DrachtioAgent extends EventEmitter {
  ...
  get idle(): boolean {
    ...
    this.mapServer.forEach((obj: any, socket: any) => {
  ...
  _makeRequest(params: any): void {
    ...
    obj.pendingRequests.set(msgId, (token: string[], msg: string) => {
      if (token[0] === 'OK') {
        ...
        const req = new Request(new SipMessage(msg as any), meta) ;
        (req as any).agent = this ;
        (req as any).socket = obj.socket ;
        if (params.options.auth) {
          (req as any).auth = params.options.auth ;
          (req as any)._originalParams = params ;
        }
  ...
  sendResponse(res: any, opts: any, callback: any, fnAck: any): void {
    ...
    if ((callback && typeof callback === 'function') || fnAck) {
      ...
      obj.pendingRequests.set(msgId, (token: string[], msg: string, meta: any) => {
        obj.pendingRequests.delete(msgId) ;
        if ('OK' !== token[0]) { return callback(token[1]) ; }
        const responseMsg = new SipMessage(msg as any) ;
        res.meta = meta ;
        if (callback) {
  ...
  sendAck(method: string, dialogId: string, req: any, res: any, opts: any, callback: any): void {
    ...
    (m as any).method = method as any;
    (m as any).uri = req.uri ;
    opts = opts || {} ;
    ...
    if (callback) {
      obj.pendingRequests.set(msgId, (token: string[], msg: string, meta: any) => {
        ...
        callback(null, new SipMessage(msg as any)) ;
      }) ;
    }
  }
  ...
  routeVerbs(socket: any): void {
    this.verbs.forEach((obj: any, verb: string) => {
      if (obj.sent === true) {
        return ;
      }
  ...
  _onMsg(socket: net.Socket | tls.TLSSocket, msg: string): void {
    ...
    let res: any, sr: any, rawMsg: string | undefined;
    ...
    switch (token[1]) {
      case 'sip':
        ...
        try {
          sipMsg = new SipMessage(rawMsg as any) ;
        } catch (err) {
        ...
        if (token.length > 9) {
          ...
          if ('network' === source && sipMsg.type === 'request') {
            ...
            (req as any).res = res ;
            (res as any).req = req ;
            (req as any).agent = (res as any).agent = this ;
            (req as any).socket = (res as any).socket = socket ;
            ...
            if ('INVITE' === req.method) {
              ...
              obj.pendingSipRequests.forEach((value: any, _key: any) => {
            ...
            if (obj.pendingSipRequests.has(transactionId)) {
              ...
              res.msg = sipMsg ;
              res.meta = meta ;
              (res as any).req = sr.req ;
              (res as any).socket = (res.req as any).socket = socket ;
              ...
              if (obj.pendingSipAuthRequests.has(cid)) {
                ...
                client.authenticate((err: Error | null, req?: SrfRequest) => {
                  if (!req) {
                    sr.req.emit('response', res, ack);
                    return;
                  }
                  // move all listeners from the old request to the new one we just generated
                  res.req.listeners('response').forEach((l: any) => { req.on('response', l) ; }) ;
      ...
      case 'cdr:stop':
        ...
        rawMsg = msg.slice(pos + 2) ;
        const cdrSipMsg = new SipMessage(rawMsg as any) ;
...
(DrachtioAgent.prototype as any).uac = DrachtioAgent.prototype.request ; // alias
