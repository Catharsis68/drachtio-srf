...
app._init = function() {
  const client = new DrachtioAgent(this);
...
app.endSession = function(socketHolder: any) {
...
app.set = function(prop: string, value: any) {
  this.params[prop] = value ;
};

app.get = function(prop: string) {
  ...
  function next(err?: Error) {
    ...
    if (!layer || res.finalResponseSent) {
      ...
      if (err) {
        ...
        // respect err.status
        if ((err as any).status) { res.status = (err as any).status; }

        // production gets a basic error message
        const msg = sipStatus[res.status] ;
